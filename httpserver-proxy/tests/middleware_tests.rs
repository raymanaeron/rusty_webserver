...
use httpserver_config::{
    ProxyRoute, LoadBalancingStrategy, MiddlewareConfig, HeaderMiddlewareConfig,
    RateLimitConfig, TransformConfig, RequestTransformConfig, ResponseTransformConfig,
    TextReplacement, AuthMiddlewareConfig, ApiKeyConfig, CompressionConfig
};
...
async fn test_compression_middleware() {
    ...
    let large_body = "This is a large response body that should be compressed because it exceeds the threshold size of 10 bytes.";
    ...
    let compressed_body = axum::body::to_bytes(processed_response.into_body(), usize::MAX).await.unwrap();
