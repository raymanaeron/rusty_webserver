...
async fn test_websocket_health_monitoring_setup() {
    ...
    // Create load balancer
    let load_balancer = Arc::new(LoadBalancer::new(targets, LoadBalancingStrategy::LeastConnections));
...
async fn test_health_callback_mechanism() {
    ...
    // Create load balancer
    let load_balancer = Arc::new(LoadBalancer::new(targets, LoadBalancingStrategy::WeightedRoundRobin));
