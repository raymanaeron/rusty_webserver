use axum::{ extract::Request, response::Response, http::{ HeaderName, HeaderValue }, body::Body };
use std::{
    collections::HashMap,
    sync::{ Arc, Mutex },
...
impl MiddlewareProcessor {
    ...
    fn check_rate_limit(
        ...
        if active_count >= rate_config.max_concurrent {
            ...
            return Err(
                MiddlewareError::RateLimitExceeded("Too many concurrent connections".to_string())
            );
        }
    ...
    fn apply_auth_headers(
        ...
        if let Some(token) = &auth_config.bearer_token {
            ...
            headers.insert(
                ...
                HeaderValue::from_str(&auth_value).map_err(|e|
                    MiddlewareError::AuthError(format!("Invalid bearer token: {}", e))
                )?
            );
        ...
        if let Some(basic_auth) = &auth_config.basic_auth {
            ...
            headers.insert(
                "authorization",
                HeaderValue::from_str(&auth_value).map_err(|e|
                    MiddlewareError::AuthError(format!("Invalid basic auth: {}", e))
                )?
            );
        ...
        if let Some((header_name, header_value)) = &auth_config.custom_auth_header {
            headers.insert(
                HeaderName::from_bytes(header_name.as_bytes()).map_err(|e|
                    MiddlewareError::AuthError(format!("Invalid header name: {}", e))
                )?,
                HeaderValue::from_str(header_value).map_err(|e|
                    MiddlewareError::AuthError(format!("Invalid header value: {}", e))
                )?
            );
        ...
        if let Some(api_key_config) = &auth_config.api_key {
            headers.insert(
                HeaderName::from_bytes(api_key_config.header_name.as_bytes()).map_err(|e|
                    MiddlewareError::AuthError(format!("Invalid API key header name: {}", e))
                )?,
                HeaderValue::from_str(&api_key_config.key_value).map_err(|e|
                    MiddlewareError::AuthError(format!("Invalid API key value: {}", e))
                )?
            );
            tracing::debug!(
    ...
    fn apply_request_headers(
        ...
        for (name, value) in &header_config.request_headers {
            let header_name = HeaderName::from_bytes(name.as_bytes()).map_err(|e|
                MiddlewareError::HeaderError(format!("Invalid header name '{}': {}", name, e))
            )?;
            let header_value = HeaderValue::from_str(value).map_err(|e|
                MiddlewareError::HeaderError(format!("Invalid header value '{}': {}", value, e))
            )?;
        ...
        if let Some(host_override) = &header_config.override_host {
            headers.insert(
                "host",
                HeaderValue::from_str(host_override).map_err(|e|
                    MiddlewareError::HeaderError(format!("Invalid host override: {}", e))
                )?
            );
    ...
    async fn transform_request_body(
        ...
        let body_bytes = axum::body
            ::to_bytes(body, usize::MAX).await
            .map_err(|e|
                MiddlewareError::TransformError(format!("Failed to read request body: {}", e))
            )?;
    ...
    async fn transform_response_body(
        ...
        let body_bytes = axum::body
            ::to_bytes(body, usize::MAX).await
            .map_err(|e|
                MiddlewareError::TransformError(format!("Failed to read response body: {}", e))
            )?;
    ...
    fn transform_json_body(
        ...
        let mut json: Value = serde_json
            ::from_str(&body_string)
        ...
        serde_json
            ::to_string(&json)
            .map_err(|e|
                MiddlewareError::TransformError(format!("Failed to serialize JSON: {}", e))
            )
    }
    ...
    async fn apply_compression(
        ...
        let body_bytes = axum::body
            ::to_bytes(body, usize::MAX).await
            .map_err(|e|
                MiddlewareError::CompressionError(format!("Failed to read response body: {}", e))
            )?;
        ...
        if compression_config.gzip {
            ...
            let mut encoder = GzEncoder::new(
                Vec::new(),
                Compression::new(compression_config.level)
            );
            encoder
                .write_all(&body_bytes)
                .map_err(|e|
                    MiddlewareError::CompressionError(format!("Gzip compression failed: {}", e))
                )?;
            let compressed = encoder
                .finish()
                .map_err(|e|
                    MiddlewareError::CompressionError(format!("Gzip finish failed: {}", e))
                )?;
            ...
            tracing::debug!(
                ...
                ratio = format!(
                    "{:.2}%",
                    ((compressed.len() as f64) / (body_bytes.len() as f64)) * 100.0
                ),
