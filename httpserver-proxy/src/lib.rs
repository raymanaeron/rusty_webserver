...
mod tests {
    ...
    fn test_global_wildcard() {
        let routes = vec![
            create_test_route("*", "http://localhost:3000"),
        ];
    ...
    fn test_pattern_compilation() {
        // Test exact pattern
        let route = create_test_route("/health", "http://localhost:3000");
        let pattern = RouteMatcher::compile_pattern(&route);
        assert!(!pattern.has_wildcard);
        assert_eq!(pattern.parts.len(), 1);
        
        // Test wildcard pattern
        let route = create_test_route("/api/*", "http://localhost:3000");
        let pattern = RouteMatcher::compile_pattern(&route);
        assert!(pattern.has_wildcard);
        assert_eq!(pattern.parts.len(), 2);
        
        // Test global wildcard
        let route = create_test_route("*", "http://localhost:3000");
        let pattern = RouteMatcher::compile_pattern(&route);
        assert!(pattern.has_wildcard);
        assert_eq!(pattern.parts.len(), 1);
    }
