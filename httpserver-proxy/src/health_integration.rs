...
impl HealthCheckIntegration {
    ...
    pub async fn start_http_health_monitoring(
        &mut self,
        config: HttpHealthConfig
    ) -> Result<(), String> {
        // Extract target URLs from the load balancer
        let targets: Vec<String> = self.load_balancer
            .targets()
            .iter()
        ...
        println!(
            "HTTP health monitoring started for {} targets",
    ...
    pub async fn start_websocket_health_monitoring(
        &mut self,
        ...
        self.websocket_health_monitor = Some(monitor);

        println!(
            "WebSocket health monitoring started for {} targets",
            self.load_balancer.targets().len()
        );
    ...
    pub fn get_health_summary(&self) -> HealthSummary {
        ...
        HealthSummary {
            ...
            monitoring_enabled: self.websocket_health_monitor.is_some() ||
            self.http_health_monitor.is_some(),
        }
    }
}
...
impl std::fmt::Display for HealthSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Health Summary: {}/{} healthy targets, monitoring: {}",
