...
async fn setup_test_files(temp_dir: &TempDir) -> PathBuf {
    ...
    // Create test files
    fs::write(temp_path.join("index.html"), 
        "<!DOCTYPE html><html><body><h1>Home Page</h1></body></html>").await.unwrap();
    
    fs::write(temp_path.join("test.css"), 
        "body { background-color: #f0f0f0; }").await.unwrap();
    
    fs::write(temp_path.join("test.js"), 
        "console.log('Hello World');").await.unwrap();
    
    fs::write(temp_path.join("test.json"), 
        r#"{"message": "test data"}"#).await.unwrap();
    
    // Create a subdirectory with files
    fs::create_dir(temp_path.join("assets")).await.unwrap();
    fs::write(temp_path.join("assets").join("logo.svg"), 
        r#"<svg><circle r="10" /></svg>"#).await.unwrap();
...
async fn test_serve_css_file() {
    ...
    assert!(content_type.to_str().unwrap().contains("text/css"));    let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
    let content = String::from_utf8(body.to_vec()).unwrap();
...
async fn test_spa_fallback_to_index() {
    ...
    let response = app.oneshot(request).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);    let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
    let content = String::from_utf8(body.to_vec()).unwrap();
